...
function ldapScript(secret:{controller:string;useLdaps:boolean;port:number;baseDn:string;username:string;password:string},listUsers:boolean){
  const common=[
    ...
    "if($scheme -eq 'LDAPS'){ $authType = $authType -bor [System.DirectoryServices.AuthenticationTypes]::SecureSocketsLayer }",
  ...
  if(!listUsers)return [...common,
    ...
    "[pscustomobject]@{ok=$true;path=$path;baseDn=$baseDn;controller=$controller;port=[int]$port;protocol=$scheme} | ConvertTo-Json -Compress",
  ].join("\r\n");
  return [...common,
...
function ldapSingleUserScript(
  ...
  const lines=[
    ...
    `$scheme='${secret.useLdaps?'LDAPS':'LDAP'}'`,
...
export function startDomainSyncScheduler(){
  if(timer)return;const scheduleNext=async()=>{const row=await settingsRow();const minutes=Math.max(1,Math.min(1440,row.domainSyncIntervalMinutes||60));
    timer=setTimeout(async()=>{try{if((await settingsRow()).domainSyncEnabled)await syncDomainUsers();}catch(error){console.error('[DOMAIN SYNC]',error);
      ...
      finally{timer=null;void scheduleNext();}},Math.min(minutes*60_000,15_000));timer.unref?.();};void scheduleNext();
}
