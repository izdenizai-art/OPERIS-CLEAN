...
async function syncRemoteAccessFirewall(mode: RemoteAccessMode) {
  ...
  const script = [
    ...
    "Get-NetFirewallRule -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like 'Operis Enterprise TCP *' } | Remove-NetFirewallRule -ErrorAction SilentlyContinue",
    `$names=@(${escapedNames})`,
